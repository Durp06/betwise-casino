/**
 * HoldemRulesModal.tsx — short "How to Play" popup for Multiplayer Hold'em
 * (live cash ring-game Texas Hold'em against real players).
 *
...
interface HoldemRulesModalProps {
  onClose: () => void;
}

export default function HoldemRulesModal({ onClose }: HoldemRulesModalProps) {
  useEffect(() => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("How to Play Multiplayer Hold'em")}
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <h2 className="font-display text-3xl tracking-wider text-ink leading-none">
            {t("How to Play Multiplayer Hold'em")}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="font-flavor text-sm leading-relaxed">
          {t(
            "Win chips from real players at a live cash table — either by showing the best five-card hand at showdown, or by betting everyone else out of the pot.",
          )}
        </p>
        ...
        <section className="flex flex-col gap-1">
          ...
          <ul className="font-flavor text-sm leading-relaxed flex flex-col gap-1">
            <li>{t("Cash ring game: buy in for the table's range — your chips are bankroll you can win or lose.")}</li>
            <li>{t("Blinds are fixed (they don't rise) and the dealer button rotates each hand.")}</li>
            <li>{t("All-ins create side pots; a bet nobody calls is returned to you.")}</li>
          </ul>
        </section>
        ...
        <p className="font-flavor text-sm leading-relaxed bg-ink/5 ink-outline rounded-lg p-3">
          ...
          {t("Position is everything — acting later on the flop, turn, and river gives you more information and control.")}
        </p>
